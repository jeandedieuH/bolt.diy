...
- **`npm run start`**: Runs the built application locally using Wrangler Pages.
- **`npm run preview`**: Builds and runs the production build locally.
- **`npm test`**: Runs the test suite using Vitest.
- **`npm run typecheck`**: Runs TypeScript type checking.
- **`npm run typegen`**: Generates TypeScript types using Wrangler.
- **`npm run deploy`**: Deploys the project to Cloudflare Pages.
